...
//		List<String> topics = Collections.singletonList("wikimedia");
		...
		try (elasticSearchClient; consumer) {
			...
			if (!isIndexExist) {
				System.out.println("wikimedia not index exist");
				CreateIndexRequest createIndexRequest = new CreateIndexRequest("wikimedia");
				elasticSearchClient.indices().create(createIndexRequest, RequestOptions.DEFAULT);
			} else {
				System.out.println("wikimedia index exist");
			}

			System.out.println("Begin while to instert data");
			consumer.subscribe(Collections.singletonList("wikimedia_recentchange"));
			while (true) {
				System.out.println("consumers.poll");
				ConsumerRecords<String, String> records = consumer.poll(Duration.ofMillis(3000));

				int recordCount = records.count();

				System.out.println("Receive: " + recordCount + " record(s)");
...
//					String id = record.topic()+"_"+record.partition()+"_"+record.offset();
					...
					try {
						...
						System.out.println("_doc/Id "+response.getId());
					} catch (Exception e) {
